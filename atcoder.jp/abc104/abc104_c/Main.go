...
var fsc = NewFastScanner()

func main() {
	D, G := fsc.NextInt(), fsc.NextInt()
	bouns := []int{}
	problemNums := []int{}
	for i := 0; i < D; i++ {
		p, c := fsc.NextInt(), fsc.NextInt()
		problemNums = append(problemNums, p)
		bouns = append(bouns, c)
	}
	maxBit := 1 << uint(D)
	result := 1 << uint(20)
	for bit := 0; bit < maxBit; bit++ {
		...
		for i := 0; i < D; i++ {
			if ((bit >> uint(i)) & 1) == 1 {
				score += bouns[i] + 100*(i+1)*problemNums[i]
				problemNum += problemNums[i]
			}
		}
		if score >= G {
			result = IntMin(result, problemNum)
			continue
		}
		for i := D - 1; i >= 0; i-- {
			if ((bit >> uint(i)) & 1) == 1 {
