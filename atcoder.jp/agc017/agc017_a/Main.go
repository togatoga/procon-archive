...
func main() {
	...
	var result int64
	result = 0
	// fmt.Println(zeroCnt, oneCnt)
	if P == 0 { //zero
		//oneCnt is zero
		for i := 0; i <= oneCnt; i += 2 {
			result += calC(oneCnt, i) * powTwo
		}
	} else { //1
		for i := 1; i <= oneCnt; i += 2 {
			result += calC(oneCnt, i) * powTwo
			// fmt.Println(oneCnt, i, calC(oneCnt, i))
		}
	}
	fmt.Println(result)
}
