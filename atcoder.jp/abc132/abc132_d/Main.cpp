...
int main()
{
    ...
    //cout << mod_util.get_C(2000, 1000) << endl;
    ll a = N - K;
    ll b = K;
    //N + 2
    ...
    for (int i = 1; i <= K; i++)
    {
        //N + 2Ci
        ll tmp = 0;
        tmp = mod_util.get_C(a + 1, i) % MOD;
        tmp %= MOD;
        //K + 2C i - 1
        tmp *= mod_util.get_C(b - 1, i - 1) % MOD;
        tmp %= MOD;
        //tmp *= mod_util.get_inv_fact(i) % MOD;
        //tmp %= MOD;
        //cerr << a << " " << b << " " << mod_util.get_C(3, 1) << endl;
        ///ut << mod_util.get_C(a + 1, i) << " " << mod_util.get_C(b + 1, i - 1) << endl;
        tmp %= MOD;
        cout << tmp << endl;
    }
    return 0;
