...
fn solve() {
    ...
    let mut sum = n * (n - 1) / 2;
    let mut results = queries.iter().map(|&(x, y)| {
        if ut.is_same_set(x, y) {
            ut.unite(x, y);
            return sum;
        }
        let x_size = ut.set_size(x);
        let y_size = ut.set_size(y);
        ut.unite(x, y);
        sum -= x_size * y_size;
        sum
    }).collect::<Vec<usize>>();
    results.reverse();
    results.push(n * (n - 1) / 2);
    for x in 1..results.len() {
        println!("{}", results[x]);
    }
}
