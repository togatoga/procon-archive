...
fn solve() {
    ...
    let h: usize = sc.read();
    let w: usize = sc.read();
    let mut a: Vec<Vec<char>> = (0..h).map(|_| sc.chars()).collect();
    ...
    let mut result = 0;
    while let Some((y, x, cnt)) = stack.pop_front() {
        result = cnt;
        //eprintln!("{} {}", y, x);
        for k in 0..4 {
            if let Some((ny, nx)) = utils::try_adj(y, x, k, h, w) {
                if a[ny][nx] == '.' {
                    a[ny][nx] = '#';
                    stack.push_back((ny, nx, cnt + 1));
                }
            }
        }
    }
