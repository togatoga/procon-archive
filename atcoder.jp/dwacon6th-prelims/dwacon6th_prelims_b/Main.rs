...
fn solve() {
    ...
    let mut tmp1 = ModInt::new(1);
    for i in 0..n - 1 {
        tmp1 *= i + 1;
    }
    let mut tmp = ModInt::new(0);
    for (idx, v) in x.windows(2).enumerate() {
        let c = ModInt::new(v[1] - v[0]);
        result += c * tmp1 / ModInt::new(idx + 1) + c * tmp;
        tmp += tmp1 / ModInt::new(idx + 1);
        //println!("{:?} {:?} {:?}", result, v, tmp);
    }
    println!("{}", result.0);
}
